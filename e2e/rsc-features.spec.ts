...
test.describe("RSC Features", () => {
  ...
  test("Suspense: agents skeleton appears then replaced", async ({ page }) => {
    ...
    await expect(page.locator(".skeleton").first()).toBeVisible({ timeout: 5000 }).catch(() => {});
    await expect(page.locator("text=Sentinel").first()).toBeVisible({ timeout: 15000 });
  });
  ...
  test("full chain: agents page loads agents from backend", async ({ page }) => {
    await page.goto("/agents");
    await expect(page.locator("text=Sentinel").first()).toBeVisible({ timeout: 15000 });
    await expect(page.locator("text=Dispatch").first()).toBeVisible({ timeout: 10000 });
