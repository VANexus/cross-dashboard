import type { NextRequest } from "next/server";
import { withDb } from "@/lib/server/api-helpers";
import { success, error, badRequest, methodNotAllowed } from "@/lib/server/api-response";
import { parseBody, contentAuditSchema } from "@/lib/server/api-validation";
import { ContentService, ContentMCPError } from "@/lib/server/services";

const service = new ContentService();

export const POST = withDb(async (request: NextRequest) => {
  const parsed = parseBody(contentAuditSchema, await request.json());
  if (!parsed.success) return badRequest(parsed.error);
  try {
    const result = await service.auditDraft({ id: parsed.data.id });
    return success(result);
  } catch (err) {
    if (err instanceof ContentMCPError) {
      return error(err.message, err.category === "skill" ? 422 : 503);
    }
    return error(err instanceof Error ? err.message : "审计失败", 500);
  }
});

export { methodNotAllowed as GET };
export { methodNotAllowed as PATCH };
export { methodNotAllowed as DELETE };
