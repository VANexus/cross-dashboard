    ...
    <PageTransition className="space-y-6">
      <div className="data-grid grid-cols-2 md:grid-cols-4">
        ...
        <div className="flex flex-col gap-1 p-4">
          ...
          <div className="flex items-baseline gap-2">
            <AnimatedNumber value={12} className="text-2xl font-bold" />
        ...
        <div className="flex flex-col gap-1 p-4">
          ...
          <div className="flex items-baseline gap-2">
            <AnimatedNumber value={2} className="text-2xl font-bold" />
                ...
                <div className="flex gap-0.5">
                  {Array.from({ length: 12 }).map((_, j) => (
                    <div
                      ...
                      style={{ height: `${8 + Math.random() * 12}px`, opacity: j > 9 ? 0.3 : 0.7 + Math.random() * 0.3 }}
                    />
                  ))}
                </div>
                <span className="text-[10px] text-muted-foreground w-12 text-right">
                  {99 - i}.{Math.floor(Math.random() * 9)}%
                </span>
              </div>
            ))}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-end justify-between">
              <AnimatedNumber value={5800} prefix="$" className="text-2xl font-bold" />
              <Sparkline data={salesTrend} width={100} height={32} color="var(--success)" />
            </div>
            <p className="text-[11px] text-emerald-500 mt-1">↑ 12.4% vs 上周</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="flex items-end justify-between">
              <AnimatedNumber value={20} suffix="%" className="text-2xl font-bold" />
              <Sparkline data={acosTrend} width={100} height={32} color="var(--primary)" />
            </div>
            <p className="text-[11px] text-emerald-500 mt-1">↓ 37.5% vs 上月</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="flex items-end justify-between">
              <AnimatedNumber value={16.2} suffix="%" decimals={1} className="text-2xl font-bold" />
              <Sparkline data={conversionTrend} width={100} height={32} color="var(--info)" />
            </div>
            <p className="text-[11px] text-emerald-500 mt-1">↑ 33.9% vs 上月</p>
          </CardContent>
        </Card>
      </div>
    </PageTransition>
