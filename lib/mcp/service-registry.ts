...
/** 从环境变量解析预设端点（目录请求失败时的兜底路径；正常链路走 loadEndpointsFromCatalog） */
...
/**
 * 服务化零配置：向 BFF 要「集群服务目录」的浏览器脱敏视图。
 * cluster 形态返回的是边缘同源反代地址（浏览器可达的内网 svc 地址不会下发）。
 */
...
async function loadEndpointsFromCatalog(): Promise<ServiceEndpoint[] | null> {
  try {
    const res = await fetch("/api/cluster/services", { cache: "no-store" });
    if (!res.ok) return null;
    const json = (await res.json()) as { data?: { services?: CatalogView[] } };
    const services = json.data?.services;
    if (!services?.length) return null;

    const endpoints: ServiceEndpoint[] = [];
    const mcp = services.find((s) => s.id === "flowmind.mcp");
    const mcpUrl = mcp?.browserUrl ?? mcp?.url;
    if (mcpUrl) {
      endpoints.push({
        id: "flowmind-primary",
        name: mcp!.name,
...
export const useServiceRegistry = create<ServiceRegistryState>((set, get) => ({
  ...
  initialize: async () => {
    // 服务化：优先向集群服务目录要端点；失败才回落 env 预设
    const catalog = await loadEndpointsFromCatalog();
    const presets = catalog ?? loadPresetEndpoints();
