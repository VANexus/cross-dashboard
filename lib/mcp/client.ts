 ...
 *
 * 端点零配置：URL 由集群服务目录解析（lib/cluster）——
 *   集群内 = http://flowmind-mcp.core-api.svc:8001/mcp（服务化部署），
 *   开发机 = 本机 flowmind-mcp-http（127.0.0.1:8001/mcp），
 *   逃生门 = 显式 FLOWMIND_MCP_URL 覆盖。UI 设置页不再有地址输入框。
...
import { flowmindMcpUrl } from "@/lib/cluster";
...
export function getMCPConfig(): ContentMCPConfig {
  return {
    // 零配置：集群内 = flowmind-mcp.core-api.svc；开发机 = 本机 8001；FLOWMIND_MCP_URL 显式覆盖
    url: flowmindMcpUrl(),
