/**
 * FlowMind RAK — 0001_init：纯 PostgreSQL DDL 初始化迁移
 *
 * 来源：supabase/migrations/ 全量转换合并（00001_init_schema … 00013_saas_groundwork
 * + _apply_pending_20260902.sql + _apply_pending_p0_specs_20260902.sql；两个
 * _apply_pending 脚本内容与其对应的编号迁移重复，已去重）。
 *
 * 转换规则：
 *   保留：CREATE TABLE（后续 ALTER TABLE ADD COLUMN 已折叠进建表语句，
 *         列/默认值/主键/外键/唯一约束齐全）、CREATE INDEX / UNIQUE INDEX。
 *         全部 IF NOT EXISTS。类型（text/timestamptz/jsonb/numeric/real 等）原样保留。
 *   丢弃：ENABLE ROW LEVEL SECURITY、CREATE/DROP POLICY、GRANT/REVOKE/ALTER DEFAULT
 *         PRIVILEGES、auth.uid()、storage.*（含 storage.buckets 种子 INSERT）、
 *         ALTER PUBLICATION、所有 INSERT 种子数据与 DELETE 演示数据清理（数据由应用层负责）、
 *         COMMENT ON、CREATE FUNCTION / TRIGGER（set_updated_at 自动时间戳、
 *         protect_builtin_image_skills 删除保护、trim_trend_snapshots 清理、
 *         b2b_daily_refresh pg_cron/pg_net 调度等——改由应用层实现）。
 *   折叠：wf_image_skills 的 is_builtin / template_type 列与
 *         chk_wf_image_skills_template_type CHECK 约束（来自 00003/00004/00005）；
 *         wf_workflow_specs / wf_page_specs 的 workspace_id / created_by 列（来自 00013）。
 */
...
-- is_builtin / template_type 列与 CHECK 约束折叠自 00003/00004/00005
CREATE TABLE IF NOT EXISTS wf_image_skills (
  ...
  CONSTRAINT chk_wf_image_skills_template_type
...
-- ========== AI 动态产物 spec（00012 + 00013 归属列折叠） ==========
