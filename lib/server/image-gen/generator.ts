...
export class ImageGenerator {
  ...
  /**
   * 生图 URL 组装（容错重复路径）：
   * baseUrl 可能是源站（https://x）、含 /v1、或已是完整 …/v1/images/generations。
   * 统一规避重复拼接：若已以 /v1/images/generations 结尾则直接使用，否则补全。
   */
  ...
  private async generateWithDallE(
    ...
    if (isSiliconFlow) {
      ...
      // OpenAI 兼容分支：只发稳定参数。多数端点（含 MPT-Image-2 / ciyuansky）拒绝
      // response_format，缺省即可（端点自行决定返回 url 或 b64_json，generator 两者都兼容）。
      body = {
        model,
        prompt,
        n: count,
