...
import type { EvolutionRecord, Pagination } from "@/lib/shared/types";
...
export class EvolutionService {
  ...
  async getById(id: string): Promise<(EvolutionRecord & { beforeMetrics?: EvolutionRecord["metrics"] }) | null> {
    return await repo.getEvolutionById(id);
  }

  /** Mongo 中的进化运行审计轨迹（五阶段 + before/after）。 */
  async getRunTrace(recordId: string) {
    const { getEvolutionRun } = await import("../db/mongo-stores");
    return await getEvolutionRun(recordId);
  }
