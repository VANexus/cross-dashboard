...
// 「UI 即工具」动作注册表：navigate/refresh/openDrawer/highlight 等通用动作 +
// 各页面经 registerPageActions 注册的页面动作。Agent 抽屉收到 ui_action tool call 后
// 按 id 在此路由到本地 execute（本地 zod 校验），结果经 addToolResult 回传续推。
import { z } from 'zod';
...
// ── 注册表：module-level Map（通用 + 当前页）────────────────────────
const globalRegistry = new Map<string, UIActionDef>();
const pageRegistry = new Map<string, UIActionDef>();

/** 注册全局通用动作（所有页面可用；应用启动时由 Agent 抽屉挂载一次）。 */
export function registerGlobalActions(actions: UIActionDef[]): void {
  for (const a of actions) globalRegistry.set(a.id, a);
}
...
export function registerPageActions(actions: UIActionDef[]): void {
  ...
  for (const a of actions) pageRegistry.set(a.id, a);
}
...
export function getPageActions(): UIActionDef[] {
  return [...globalRegistry.values(), ...pageRegistry.values()];
}

/** 按 id 查找动作（通用 + 当前页）。 */
export function getActionById(id: string): UIActionDef | undefined {
  return pageRegistry.get(id) ?? globalRegistry.get(id);
}
...
/** 校验并执行动作，返回结果摘要或错误说明。 */
export async function runAction(id: string, params: Record<string, unknown> = {}): Promise<string> {
  const action = getActionById(id);
  if (!action) {
    return `未注册的 UI 动作：${id}（当前可用：${getPageActions().map((a) => a.id).join(', ') || '无'}）`;
  }
  let p = params;
  if (action.schema) {
...
/** 挂载 window.__agentUI = { list, execute }（生产环境跳过）。 */
export function installAgentTestHook(): void {
  if (typeof window === 'undefined' || process.env.NODE_ENV === 'production') return;
  (window as unknown as Record<string, unknown>).__agentUI = {
    list: () => getPageActions().map(({ id, description }) => ({ id, description })),
    execute: (id: string, params?: Record<string, unknown>) => runAction(id, params),
  };
}
